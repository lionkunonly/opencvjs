async function loadOpenCV(paths, onloadCallback) {
    ...
    if (simdSupported && threadsSupported && threadsSimdPath != "") {
        ...
        if (threadsSupported && threadsSimdPath === "") {
            log("The browser supports simd and threads, but the path of OpenCV.js with simd and threads optimization is empty");
        }
        OPENCV_URL = simdPath;
        ...
        if (simdSupported) {
            log("The browser supports simd optimization, but the path of OpenCV.js with simd optimization is empty");
        }

        if (threadsSupported) {
            log("The browser supports threads optimization, but the path of OpenCV.js with threads optimization is empty");
        }

        OPENCV_URL = wasmPath;
        log("The OpenCV.js for wasm is loaded now");
    } else if (wasmSupported) {
        log("The browser supports wasm, but the path of OpenCV.js for wasm is empty");
    }
